...
<footer
    class="{{ Str::contains(request()->fullUrl(), 'backend') ? 'lg:pl-[20rem] lg:pr-8' : 'max-w-6xl' }} my-6 text-center md:flex text-subtext0 items-center justify-between mx-auto">
        ...
        <div>
            <form action="{{ route('set-theme') }}" method="post">
                ...
                <select name="theme" id="theme" class="bg-crust text-text px-1 py-0.5 text-sm w-28">
                    ...
                    <option {{ session('theme') === 'macchiato' ? 'selected' : '' }} value="macchiato">Macchiato
                    </option>
                ...
                <button type="submit"
                        class="border border-blue/60 px-2 py-0.5 text-sm hover:bg-crust transition duration-150 ease-in">
                    Save
                </button>
            </form>
        </div>
