...
<template>
    <div role="listitem" class="bg-crust p-5 shadow-sm shadow-surface0">
        <div class="inline-flex justify-between w-full">
            <h4 class="text-xl m-0 p-0 text-text font-black">{{ comment.author.name }}</h4>
        ...
        <div class="inline-flex space-x-2 items-baseline">
            ...
            <time v-if="moment().diff(comment.created_at, 'days') <= 7"
                  class="text-overlay1 font-semibold text-sm"
                  :datetime="comment.created_at"
                  :title="moment(comment.created_at).format('Do MMM YYYY [at] hh:mma')">
                {{ moment(new Date()).from(comment.created_at, true) }} ago
            </time>
        </div>
        <div role="article" class="mt-4 prose prose-blue prose-invert font-medium" v-html="comment.comment" />
    </div>
            ...
            <modal @close="showDeleteModal" :show="showingModal" :closeable="true">
                <div class="px-6">
                    ...
                    <div class="text-red inline-flex space-x-4 items-center">
                        <div class="text-xl bg-red/60 px-3 py-2 rounded-full">
                            <i class="fa fa-triangle-exclamation text-white" />
                        </div>
                        <span class="text-text text-xl">Are you sure you want to delete this comment?</span>
                    </div>
