        ...
        <Suspense>
            <div>
                ...
                <div class="mb-2 flex items-center space-x-4" v-if="!!activeTag">
                    ...
                    <div>
                        <button type="button" @click.prevent="activeTag = null"
                                class="relative inline-flex items-center justify-center p-0.5 mb-2 me-2 overflow-hidden text-xs font-medium text-gray-900 group bg-gradient-to-br from-purple-600 to-blue-500 group-hover:from-purple-600 group-hover:to-blue-500 hover:text-white dark:text-white focus:ring-4 focus:outline-none focus:ring-blue-300 dark:focus:ring-blue-800">
                            <span class="relative px-3 py-1.5 transition-all ease-in duration-75 bg-white dark:bg-gray-900 group-hover:bg-opacity-0">
                                Clear Filter
                            </span>
                        </button>
                    </div>
                ...
                <div class="flex items-start space-x-4 space-y-3">
                    <div class="w-3/4">
                        <ContentList as="section" path="/" v-if="activeTag === null">
                            <template #not-found>
                                <div class="py-6 text-center h-56 shadow shadow-c-darkest bg-c-dark text-3xl flex flex-col space-y-6 items-center justify-center">
                                    <p>No articles found.</p>
                                    <p>You caught me slackin'.</p>
                                </div>
                            </template>
                                    ...
                                    <article class="entry group mb-6">
                                        ...
                                        <p class="text-sm text-gray-400 my-3">
                                            ...
                                            <time itemprop="datePublished"
                                                  :title="moment(article.created).toISOString()" class="text-gray-500">
                            ...
                            <nuxt-link :to="article._path"
                                ...
                                <article class="prose prose-invert prose-blue prose-xl entry">
                                    ...
                                    <p class="text-sm text-gray-400 mt-3">
                                        Published {{ moment(article.created).fromNow() }}
                                        <time itemprop="datePublished"
                                              :title="moment(article.created).toISOString()"
                                              class="text-gray-500">
                                            ({{ moment(article.created).format('Do MMM YYYY, h:mma') }})
                                        </time>
                                    </p>
                                    ...
                                    <li v-for="d in data.value">
                                        <ul class="space-y-2 mt-0">
                                            <li v-for="cat in d.categories" @click.prevent="setActiveTag(cat)"
                                                ...
                                                class="cursor-pointer py-1.5 px-2 hover:bg-primary-dark/70 hover:text-white hover:shadow">
                                                {{ cat }}
                                            </li>
                                        </ul>
                                    </li>
            ...
            <template #fallback>
                <div class="py-6 text-center h-56 shadow shadow-c-darkest bg-c-dark text-3xl flex items-center justify-center">
...
<style scoped>
.entry {
    @apply border border-transparent shadow shadow-c-darkest hover:shadow-c-dark bg-c-dark p-6;
